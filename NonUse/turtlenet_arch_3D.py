...
class AttentionConv(nn.Module):
    ...
    def forward(self, x):
        batch, channels, frame, height, width = x.size()
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1024, zero_init_residual=False,
        ...
        #Reconstruction
        self.norm_layer = nn.LayerNorm([512,1,8,8])
        # self.norm_layer = nn.LayerNorm([2048, 8, 8])

        #for basicblock
        self.transpose_conv_0 = nn.ConvTranspose3d(512,256,3,2,1,1)
        self.transpose_conv_1 = nn.ConvTranspose3d(256,128,3,2,1,1)
        self.transpose_conv_2 = nn.ConvTranspose3d(128,64,3,2,1,1)
        self.transpose_conv_3 = nn.ConvTranspose3d(64,32,3,2,1,1)
        self.transpose_conv_4 = nn.ConvTranspose3d(32,8,3,2,1,1)

        #self.reduce_noise = nn.Conv2d(2, 2, 1)
        self.reduce_noise = nn.Conv3d(8,2,1)
        #Reduce filter
        #self.reduce_filter_0 = nn.ConvTranspose2d(512,128,3,2,1,1)
        self.maxpool_after_cat = nn.MaxPool3d((3,1,1))
    ...
    def _forward_impl(self, x):
        ...
        x = self.reduce_noise(x).view(-1,2,256,256).cuda()
